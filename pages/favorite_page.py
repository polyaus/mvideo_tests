...
class FavoritePage(BasePage):
    ...
    def favorite_list_is_empty_from_favorite_list(self):
        ...
        assert text_empty_favorite_list.text == 'В избранном пока ничего нет', f"Favorite list is empty," \
                                                                               f"{text_empty_favorite_list.text}"
    ...
    def check_product_in_favorite_list(self):
        ...
        assert int(count_fav_products.text) == 1, f"Favorite list has more one product," \
                                                  f"{count_fav_products.text}"

    def check_two_products_in_favorite_list(self):
        count_fav_products = self.browser.find_element(*FavoritePageLocators.COUNT_PRODUCTS_IN_FAV)
        assert int(count_fav_products.text) == 2, f"Favorite list has more one product," \
                                                  f"{count_fav_products.text}"
    ...
    def delete_first_added_product_from_favorite_list(self):
        ...
        assert len(delete_fav_product_buttons) == 2, f"Delete favorite products buttons count is not two, " \
                                                     f"{len(delete_fav_product_buttons)}"
